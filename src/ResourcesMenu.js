export default function ProductMenu() {
  return (
    <div className="extended-menu product flex flex-col ">
      <div className="mt-5 bg-white p-5 rounded-lg">
        <div className="flex text-rw p-5 bg-[blue] ">
          <a href="/">Resources</a>
        </div>
        <div className="flex flex-row">
          <ul className="pr-10 flex flex-col ">
            <h3 className="flex mb-3">Products</h3>
            <li className="flex items-center">
              <a href="/product#">Employer of Record</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Contract Managment</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Global Payroll</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">HRIS Software</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Remote Relocation</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Employer of Record</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Contract Managment</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Global Payroll</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">HRIS Software</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Remote Relocation</a>
            </li>
          </ul>
          <ul className="pr-10 flex flex-col ">
            <h3 className="flex mb-3">Solutions</h3>
            <li className="flex items-center">
              <a href="/product#">Employer of Record</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Contract Managment</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Global Payroll</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">HRIS Software</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Remote Relocation</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Employer of Record</a>
            </li>
          </ul>
          <ul className="pr-10 flex flex-col ">
            <h3 className="flex mb-3">Business size</h3>
            <li className="flex items-center">
              <a href="/product#">Employer of Record</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Contract Managment</a>
            </li>

            <li className="flex items-center">
              <a href="/product#">Remote Relocation</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Employer of Record</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Contract Managment</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Global Payroll</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">HRIS Software</a>
            </li>
            <li className="flex items-center">
              <a href="/product#">Remote Relocation</a>
            </li>
          </ul>
        </div>
      </div>
    </div>
  );
}
